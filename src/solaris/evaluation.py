...
def ablation_study(
    ...
    model_no_np = SolarisFusionModel(random_seed=42, neupert_lambda=0.0).fit(train)
    ...
    model_with_np = SolarisFusionModel(random_seed=42, neupert_lambda=float(config["model"]["neupert_lambda"])).fit(train)
...
def run_full_evaluation(
    ...
    if HAS_MATPLOTLIB:
        try:
            cal_path = calibration_plot(y_true, y_prob, output_dir / "calibration_curve.png", model_name="Solaris Model")
    ...
    try:
        from solaris.features import FEATURE_COLUMNS
        train = dataset[dataset["split"] == "train"]
        model = SolarisFusionModel(random_seed=42, neupert_lambda=float(config["model"]["neupert_lambda"]))
        model.fit(train)
...
def _generate_evaluation_report(results: dict, predictions: pd.DataFrame, config: dict) -> dict:
    lines = [
        "# Project Solaris Comprehensive Evaluation Report",
        "",
