...
from solaris.anomaly import compute_anomaly_index
from solaris.config import ROOT, ensure_directories, load_config
...
from solaris.data.staleness import compute_staleness_score, add_staleness_flags
from solaris.data.windows import add_forecast_labels
from solaris.features import FEATURE_COLUMNS, add_features
from solaris.models import monte_carlo_uncertainty, train_models
...
def build_dataset(config: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if data_mode == "goes_proxy":
        ...
        from solaris.data.solexs import build_solexs_replay
...
def _add_radiation_context(predictions: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    try:
        from solaris.radiation import compute_sep_risk_index, fetch_goes_particle_data
        ...
        if satellite_risk_enabled:
            from solaris.radiation import SatelliteOrbit, assess_satellite_risk
            orbit_map = {
...
def write_reports(root: Path, dataset: pd.DataFrame, events: pd.DataFrame, predictions: pd.DataFrame, bundle, config: dict | None = None) -> None:
    ...
    if config is not None:
        try:
            from solaris.evaluation import attention_heatmap as save_attention_heatmap
            save_attention_heatmap(attention, reports / "attention_heatmap.png", title="Cross-Attention Weights")
        except Exception:
            pass

        try:
            from solaris.evaluation import run_full_evaluation
...
def _event_summary_markdown(summary: pd.DataFrame) -> str:
    return "# Project Solaris Event Summary\n\n" + _markdown_table(summary) + "\n"
...
def build_artifact_manifest(
    ...
    paths = [
        ...
        "docs/DOC-001_Project_Solaris_Constitution_v1.0.md",
...
def _feature_report(dataset: pd.DataFrame) -> str:
    lines = [
        "# Project Solaris Feature Report",
        "",
...
def _final_version_plan() -> str:
    return """# Project Solaris MVP To Final-Version Development Plan
...
def run_mvp(config_path: str | Path | None = None) -> dict[str, Path]:
    ...
    dataset_path = ROOT / "data" / "processed" / "solaris_mvp_dataset.parquet"
    prediction_path = ROOT / "reports" / "predictions" / "solaris_mvp_predictions.parquet"
...
def _run_monitoring(predictions, bundle, config) -> Path:
    ...
    try:
        ...
        if archive_cfg.get("enabled", False):
            from solaris.archive.forecast_archive import ForecastArchive
