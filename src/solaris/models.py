...
try:
    import torch
    from solaris.torch_models import DualBranchCrossAttentionGRU
except Exception:
    torch = None
...
class SolarisFusionModel:
    ...
    def fit(self, frame: pd.DataFrame) -> "SolarisFusionModel":
...
class GRUModel:
    ...
    def fit(self, train_frame: pd.DataFrame, valid_frame: pd.DataFrame, config: dict) -> "GRUModel":
        from solaris.training import train_gru_model
...
def train_models(dataset: pd.DataFrame, config: dict) -> ModelBundle:
    ...
    if architecture == "gru" and torch is not None:
        ...
        final_model = SolarisFusionModel(
